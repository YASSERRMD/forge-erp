...
func TestMemoryLeaveFlow(t *testing.T) {
	...
	if _, err := m.SetLeaveStatus(ctx, 1, l.ID, LeaveApproved, l.RowVersion); err == nil {
		t.Error("draft→approved accepted")
	}
	upd, err := m.SetLeaveStatus(ctx, 1, l.ID, LeaveSubmitted, l.RowVersion)
	if err != nil {
		t.Fatalf("submit: %v", err)
	}
	if _, err := m.SetLeaveStatus(ctx, 1, upd.ID, LeaveApproved, upd.RowVersion); err != nil {
...
func TestMemoryExpenseFlow(t *testing.T) {
	...
	if total, _ := m.ExpenseTotal(ctx, 1, r.ID); total != 2000 {
		t.Fatalf("total=%d want 2000", total)
	}
	upd, err := m.SetExpenseStatus(ctx, 1, r.ID, ExpenseSubmitted, r.RowVersion)
	if err != nil {
		t.Fatalf("submit: %v", err)
	}
	upd, err = m.SetExpenseStatus(ctx, 1, r.ID, ExpenseApproved, upd.RowVersion)
	if err != nil {
		t.Fatalf("approve: %v", err)
	}
	if err := m.AddExpenseLine(ctx, &ExpenseLine{EntityID: 1, ReportID: r.ID,
	...
	if _, err := m.SetExpenseStatus(ctx, 1, r.ID, ExpensePaid, upd.RowVersion); err != nil {
		t.Fatalf("pay: %v", err)
	}
}
...
func TestMemorySalaryFlow(t *testing.T) {
	...
	upd, err := m.SetSalaryStatus(ctx, 1, s.ID, SalaryValidated, s.RowVersion)
	if err != nil {
		t.Fatalf("validate: %v", err)
	}
	if _, err := m.SetSalaryStatus(ctx, 1, s.ID, SalaryPaid, upd.RowVersion); err != nil {
		t.Fatalf("pay: %v", err)
	}
...
func TestPGLeaveFlow(t *testing.T) {
	...
	if _, err := st.SetLeaveStatus(ctx, 1, l.ID, LeaveSubmitted, l.RowVersion); err != nil {
		t.Fatalf("submit: %v", err)
	}
}
...
func TestCrossTenantIsolation(t *testing.T) {
	...
	if _, err := m.SetLeaveStatus(ctx, 2, l.ID, LeaveSubmitted, l.RowVersion); err == nil {
	...
	if _, err := m.SetExpenseStatus(ctx, 2, r.ID, ExpenseSubmitted, r.RowVersion); err == nil {
		t.Fatal("cross-tenant SetExpenseStatus accepted")
	} else if !isNotFound(err) {
		t.Fatalf("cross-tenant SetExpenseStatus err=%v want not-found", err)
	}
	if _, err := m.ExpenseTotal(ctx, 2, r.ID); err == nil {
	...
	if err := m.CreateSalary(ctx, sal); err != nil {
		t.Fatalf("create salary: %v", err)
	}
	if _, err := m.SetSalaryStatus(ctx, 2, sal.ID, SalaryValidated, sal.RowVersion); err == nil {
