...
func TestMemoryStoreChain(t *testing.T) {
	...
	if err := st.CreateDoc(ctx, nil, prop, ym); err != nil {
		t.Fatal(err)
	}
	if prop.Ref != "PROP-202609-0001" {
		t.Fatalf("ref = %q", prop.Ref)
	}
	if _, err := st.SetStatus(ctx, nil, prop.EntityID, prop.ID, ProposalValidated); err != nil {
		t.Fatal(err)
	}
	if _, err := st.SetStatus(ctx, nil, prop.EntityID, prop.ID, ProposalBilled); err == nil {
	...
	ordDoc := &ord
	if err := st.CreateDoc(ctx, nil, ordDoc, ym); err != nil {
		t.Fatal(err)
	}
	...
	invDoc := &inv
	if err := st.CreateDoc(ctx, nil, invDoc, ym); err != nil {
		t.Fatal(err)
	}
	...
	if _, err := st.RecordPayment(ctx, nil, pay, []int64{invDoc.ID}, ym); err != nil {
		t.Fatal(err)
	}
	got, _ := st.DocByID(ctx, nil, invDoc.EntityID, invDoc.ID)
	if got.Status != InvoicePartPaid {
		t.Fatalf("status = %d want part-paid", got.Status)
	}
	bal, _ := st.InvoiceBalance(ctx, nil, invDoc.EntityID, invDoc.ID)
	if bal != 800 {
	...
	if _, err := st.RecordPayment(ctx, nil, pay2, []int64{invDoc.ID}, ym); err != nil {
		t.Fatal(err)
	}
	got, _ = st.DocByID(ctx, nil, invDoc.EntityID, invDoc.ID)
	...
	if _, err := st.RecordPayment(ctx, nil, pay3, []int64{invDoc.ID}, ym); err == nil {
...
func TestPGStoreChain(t *testing.T) {
	...
	if _, err := st.SetStatus(ctx, pool, d.EntityID, d.ID, InvoiceValidated); err != nil {
		t.Fatalf("validate: %v", err)
	}
	pay := &Payment{EntityID: 1, OrgID: 1, Amount: 1200, Currency: "USD",
		Method: "transfer", PaidAt: time.Now().UTC()}
	if _, err := st.RecordPayment(ctx, pool, pay, []int64{d.ID}, ym); err != nil {
		t.Fatalf("pay: %v", err)
	}
	bal, err := st.InvoiceBalance(ctx, pool, d.EntityID, d.ID)
	if err != nil || bal != 0 {
...
func TestPGCrossTenantInvoice(t *testing.T) {
	...
	if _, err := st.DocByID(ctx, pool, other, d.ID); err == nil {
		t.Error("cross-tenant DocByID succeeded on PG")
	}
	if _, err := st.SetStatus(ctx, pool, other, d.ID, InvoiceValidated); err == nil {
		t.Error("cross-tenant SetStatus succeeded on PG")
	}
	if _, err := st.InvoiceBalance(ctx, pool, other, d.ID); err == nil {
