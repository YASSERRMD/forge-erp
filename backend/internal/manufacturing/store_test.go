...
func TestProduceBalances(t *testing.T) {
	...
	if err := m.CreateBOM(ctx, nil, bom); err != nil {
	...
	for _, l := range []BOMLine{
		...
		l := l
		if err := m.AddLine(ctx, nil, &l); err != nil {
			t.Fatalf("add line: %v", err)
		}
	}
	if err := m.AddLine(ctx, nil, &BOMLine{EntityID: 1, BOMID: bom.ID, ComponentID: 2, Qty: 1}); err == nil {
		t.Error("duplicate component accepted")
	}
	if err := m.AddLine(ctx, nil, &BOMLine{EntityID: 1, BOMID: bom.ID, ComponentID: 1, Qty: 1}); err == nil {
		t.Error("self-reference accepted")
	}

	mo := &ManufacturingOrder{EntityID: 1, Ref: "MO-1", BOMID: bom.ID, ProductID: 1, WarehouseID: 1, Qty: 10}
	if err := m.CreateMO(ctx, nil, mo); err != nil {
		t.Fatalf("create MO: %v", err)
	}
	bad := &ManufacturingOrder{EntityID: 1, Ref: "MO-2", BOMID: bom.ID, ProductID: 2, WarehouseID: 1, Qty: 1}
	if err := m.CreateMO(ctx, nil, bad); err == nil {
		t.Error("product/BOM mismatch accepted")
	}
	upd, err := m.SetMOStatus(ctx, nil, 1, mo.ID, MOValidated, mo.RowVersion)
	if err != nil {
		t.Fatalf("validate: %v", err)
	}
	upd, err = m.SetMOStatus(ctx, nil, 1, mo.ID, MOInProgress, upd.RowVersion)
	if err != nil {
		t.Fatalf("start: %v", err)
	}
	lines, _ := m.LinesOf(ctx, nil, bom.ID)
	plan, err := PostProduce(ctx, nil, upd, lines, testLedger{ledger})
	if err != nil {
	...
	if _, err := m.MarkProduced(ctx, nil, 1, mo.ID, upd.RowVersion); err != nil {
...
func TestProduceInsufficientStock(t *testing.T) {
	...
	upd, _ := m.SetMOStatus(ctx, nil, 1, mo.ID, MOValidated, mo.RowVersion)
	upd, _ = m.SetMOStatus(ctx, nil, 1, mo.ID, MOInProgress, upd.RowVersion)
	lines, _ := m.LinesOf(ctx, nil, bom.ID)
	if _, err := PostProduce(ctx, nil, upd, lines, testLedger{ledger}); err == nil {
		t.Error("over-consumption accepted")
	}
	// Draft MO cannot produce.
	draft := *mo
	draft.Status = MODraft
	if _, err := PostProduce(ctx, nil, draft, lines, testLedger{ledger}); err == nil {
...
func TestListMOsMemory(t *testing.T) {
	...
	list, err := m.ListMOs(ctx, nil, 1, 10, 0)
...
func TestPGMOProduce(t *testing.T) {
	...
	if _, err := st.SetMOStatus(ctx, pool, 1, mo.ID, MOInProgress, upd.RowVersion); err != nil {
		t.Fatalf("start: %v", err)
	}
	lines, err := st.LinesOf(ctx, pool, bom.ID)
...
func TestCrossTenantIsolation(t *testing.T) {
	...
	if _, err := m.BOMByID(ctx, nil, 2, bom.ID); !errors.Is(err, identity.ErrNotFound) {
		t.Fatalf("cross-tenant BOMByID err=%v want not-found", err)
	}
	if _, err := m.SetBOMStatus(ctx, nil, 2, bom.ID, BOMActive, bom.RowVersion); !errors.Is(err, identity.ErrNotFound) {
	...
	if _, err := m.MOByID(ctx, nil, 2, mo.ID); !errors.Is(err, identity.ErrNotFound) {
		t.Fatalf("cross-tenant MOByID err=%v want not-found", err)
	}
	if _, err := m.SetMOStatus(ctx, nil, 2, mo.ID, MOValidated, mo.RowVersion); !errors.Is(err, identity.ErrNotFound) {
