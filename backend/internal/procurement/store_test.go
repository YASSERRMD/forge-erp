...
func TestMemoryStoreProcureChain(t *testing.T) {
	...
	if err := st.UpsertPrice(ctx, nil, &SupplierPrice{EntityID: 1, ProductID: 2, OrgID: 9, UnitNet: 6000, Currency: "USD"}); err != nil {
		t.Fatal(err)
	}
	prices, _ := st.PricesFor(ctx, nil, 1, 2, 9)
	...
	if err := st.CreateDoc(ctx, nil, ord, ym); err != nil {
		t.Fatal(err)
	}
	...
	if _, err := st.SetStatus(ctx, nil, ord.EntityID, ord.ID, Validated); err == nil {
		t.Fatal("unapproved large order validated")
	}
	approver := int64(1)
	if _, err := st.SetApproval(ctx, nil, ord.EntityID, ord.ID, approver); err != nil {
		t.Fatal(err)
	}
	if _, err := st.SetStatus(ctx, nil, ord.EntityID, ord.ID, Validated); err != nil {
		t.Fatal(err)
	...
	rcvDoc := &rcv
	if err := st.CreateDoc(ctx, nil, rcvDoc, ym); err != nil {
		t.Fatal(err)
	}
	if _, err := st.SetStatus(ctx, nil, rcvDoc.EntityID, rcvDoc.ID, Validated); err != nil {
		t.Fatal(err)
	}
	...
	sinvDoc := &sinv
	if err := st.CreateDoc(ctx, nil, sinvDoc, ym); err != nil {
		t.Fatal(err)
	}
	if _, err := st.SetStatus(ctx, nil, sinvDoc.EntityID, sinvDoc.ID, Validated); err != nil {
		t.Fatal(err)
	}
	bal, _ := st.InvoiceBalance(ctx, nil, sinvDoc.EntityID, sinvDoc.ID)
	if _, err := st.RecordPayment(ctx, nil, &SupplierPayment{EntityID: 1, OrgID: 9, Amount: bal, Currency: "USD", Method: "transfer"}, []int64{sinvDoc.ID}, ym); err != nil {
		t.Fatal(err)
	}
	got, _ := st.DocByID(ctx, nil, sinvDoc.EntityID, sinvDoc.ID)
	if got.Status != Paid {
...
func TestPGStoreProcureChain(t *testing.T) {
	...
	if _, err := st.SetApproval(ctx, pool, po.EntityID, po.ID, 1); err != nil {
		t.Fatalf("approve: %v", err)
	}
	if _, err := st.SetStatus(ctx, pool, po.EntityID, po.ID, Validated); err != nil {
		t.Fatalf("validate: %v", err)
	}
	got, err := st.DocByID(ctx, pool, po.EntityID, po.ID)
	if err != nil || got.Ref == "" {
...
func TestMemoryCrossTenantDoc(t *testing.T) {
	...
	if _, err := st.DocByID(ctx, nil, 2, ord.ID); err == nil {
		t.Error("cross-tenant DocByID succeeded")
	}
	if _, err := st.SetStatus(ctx, nil, 2, ord.ID, Validated); err == nil {
		t.Error("cross-tenant SetStatus succeeded")
	}
	if _, err := st.SetApproval(ctx, nil, 2, ord.ID, 1); err == nil {
		t.Error("cross-tenant SetApproval succeeded")
	}
	inv := &Document{EntityID: 1, Type: documents.TypeSupplierInvoice, OrgID: 9,
		Currency: "USD", RateToBase: 1000000,
		Lines: []documents.Line{{ProductID: 2, Label: "Steel", Qty: 1, UnitNet: 100}}}
	if err := st.CreateDoc(ctx, nil, inv, ym); err != nil {
		t.Fatal(err)
	}
	if _, err := st.InvoiceBalance(ctx, nil, 2, inv.ID); err == nil {
		t.Error("cross-tenant InvoiceBalance succeeded")
	}
	if _, err := st.RecordPayment(ctx, nil, &SupplierPayment{EntityID: 2, OrgID: 9, Amount: inv.Totals.Gross, Currency: "USD", Method: "transfer"}, []int64{inv.ID}, ym); err == nil {
