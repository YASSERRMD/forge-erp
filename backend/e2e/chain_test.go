...
func runChains(t *testing.T, db platform.DBTX, pst partners.Store, cst catalog.Store, sst sales.Store, procst procurement.Store, fst finance.Store) {
	...
	if err := pst.CreateOrg(ctx, db, cust); err != nil {
		t.Fatal(err)
	}
	if err := pst.CreateOrg(ctx, db, supp); err != nil {
		t.Fatal(err)
	}
	...
	if err := cst.CreateProduct(ctx, db, prod); err != nil {
		t.Fatal(err)
	}
	wh := &catalog.Warehouse{EntityID: 1, Code: "MAIN", Label: "Main", Status: 1}
	if err := cst.CreateWarehouse(ctx, db, wh); err != nil {
		t.Fatal(err)
	}
	if _, err := cst.AppendMovement(ctx, db, &catalog.StockMovement{EntityID: 1,
	...
	mustCreateSales(t, ctx, db, sst, prop, ym)
	mustStatusSales(t, ctx, db, sst, prop.ID, sales.ProposalValidated)
	mustStatusSales(t, ctx, db, sst, prop.ID, sales.ProposalSigned)
	ord := mustConvertSales(t, ctx, db, sst, *prop, documents.TypeOrder, ym)
	mustStatusSales(t, ctx, db, sst, ord.ID, sales.OrderValidated)
	shp := mustConvertSales(t, ctx, db, sst, *ord, documents.TypeShipment, ym)
	mustStatusSales(t, ctx, db, sst, shp.ID, sales.ShipmentValidated)
	if _, err := cst.AppendMovement(ctx, db, &catalog.StockMovement{EntityID: 1,
		ProductID: prod.ID, WarehouseID: wh.ID, Qty: -2,
		Reason: catalog.ReasonShipment, Ref: shp.Ref}, false); err != nil {
		t.Fatal(err)
	}
	mustStatusSales(t, ctx, db, sst, shp.ID, sales.ShipmentClosed)
	inv := mustConvertSales(t, ctx, db, sst, *shp, documents.TypeInvoice, ym)
	mustStatusSales(t, ctx, db, sst, inv.ID, sales.InvoiceValidated)
	if _, err := sst.RecordPayment(ctx, db, &sales.Payment{EntityID: 1, OrgID: cust.ID,
		Amount: inv.Totals.Gross, Currency: "USD", Method: "transfer", PaidAt: now},
		[]int64{inv.ID}, ym); err != nil {
		t.Fatal(err)
	}
	got, _ := sst.DocByID(ctx, db, inv.EntityID, inv.ID)
	if got.Status != sales.InvoicePaid {
		t.Fatalf("sales invoice status = %d", got.Status)
	}

	// Procure-to-pay.
	if err := procst.UpsertPrice(ctx, db, &procurement.SupplierPrice{EntityID: 1,
	...
	if err := procst.CreateDoc(ctx, db, po, ym); err != nil {
		t.Fatal(err)
	}
	approver := int64(1)
	if _, err := procst.SetApproval(ctx, db, po.EntityID, po.ID, approver); err != nil {
		t.Fatal(err)
	}
	if _, err := procst.SetStatus(ctx, db, po.EntityID, po.ID, procurement.Validated); err != nil {
		t.Fatal(err)
	}
	...
	rcv := &rcvDoc
	if err := procst.CreateDoc(ctx, db, rcv, ym); err != nil {
		t.Fatal(err)
	}
	if _, err := procst.SetStatus(ctx, db, rcv.EntityID, rcv.ID, procurement.Validated); err != nil {
		t.Fatal(err)
	}
	if _, err := cst.AppendMovement(ctx, db, &catalog.StockMovement{EntityID: 1,
	...
	if _, err := procst.SetStatus(ctx, db, rcv.EntityID, rcv.ID, procurement.Stage2); err != nil {
		t.Fatal(err)
	}
	...
	sinv := &sinvDoc
	if err := procst.CreateDoc(ctx, db, sinv, ym); err != nil {
		t.Fatal(err)
	}
	if _, err := procst.SetStatus(ctx, db, sinv.EntityID, sinv.ID, procurement.Validated); err != nil {
		t.Fatal(err)
	}
	bal, _ := procst.InvoiceBalance(ctx, db, sinv.EntityID, sinv.ID)
	if _, err := procst.RecordPayment(ctx, db, &procurement.SupplierPayment{EntityID: 1,
	...
	lvl, _ := cst.Level(ctx, db, prod.ID, wh.ID)
	if lvl.Qty != 108 {
	...
	for _, a := range []*finance.Account{custA, revA, vatA} {
		if err := fst.CreateAccount(ctx, db, a); err != nil {
			t.Fatal(err)
		}
	}
	j := &finance.Journal{EntityID: 1, Code: "VEN", Label: "V"}
	if err := fst.CreateJournal(ctx, db, j); err != nil {
		t.Fatal(err)
	}
	if _, err := finance.PostInvoice(ctx, db, fst, 1, j.ID, inv.Ref, now,
		custA.ID, revA.ID, vatA.ID, inv.Totals.Net, inv.Totals.VAT, "auto", nil); err != nil {
		t.Fatal(err)
	}
	if _, err := finance.PostInvoice(ctx, db, fst, 1, j.ID, sinv.Ref, now,
		custA.ID, revA.ID, vatA.ID, sinv.Totals.Net, sinv.Totals.VAT, "auto", nil); err != nil {
		t.Fatal(err)
	}
	tb, _ := fst.TrialBalance(ctx, db, 1)
	var dr, cr int64
	for _, s := range tb {
		dr += s[0]
		cr += s[1]
	...
	ents, _ := fst.EntriesByJournal(ctx, db, j.ID)
	if err := finance.VerifyChain(ents); err != nil {
		t.Fatal(err)
	}
}

func mustCreateSales(t *testing.T, ctx context.Context, db platform.DBTX, s sales.Store, d *sales.Document, ym string) {
	t.Helper()
	if err := s.CreateDoc(ctx, db, d, ym); err != nil {
		t.Fatal(err)
	}
}

func mustStatusSales(t *testing.T, ctx context.Context, db platform.DBTX, s sales.Store, id int64, to int16) {
	t.Helper()
	d, err := s.DocByID(ctx, db, 1, id)
	if err != nil {
		t.Fatal(err)
	}
	if _, err := s.SetStatus(ctx, db, d.EntityID, id, to); err != nil {
		t.Fatal(err)
	}
}

func mustConvertSales(t *testing.T, ctx context.Context, db platform.DBTX, s sales.Store, src sales.Document, to documents.DocType, ym string) *sales.Document {
	t.Helper()
	...
	out := &next
	if err := s.CreateDoc(ctx, db, out, ym); err != nil {
		t.Fatal(err)
	}
	return out
}
